...
namespace xbytechat.api
{
    public class AppDbContext : DbContext
    {
        ...
        public DbSet<BusinessPermissionOverride> BusinessPermissionOverrides { get; set; }
        // CustomFields
        public DbSet<CustomFieldDefinition> CustomFieldDefinitions { get; set; }
        public DbSet<CustomFieldValue> CustomFieldValues { get; set; }
        public DbSet<CampaignAudienceAttachment> CampaignAudienceAttachments { get; set; }
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Role>().HasData(
                new Role { Id = superadminRoleId, Name = "superadmin", Description = "Super Admin", CreatedAt = seedCreatedAt },
            ...
            modelBuilder.Entity<AudienceMember>(e =>
            {
                ...
                // ✅ Keep ONLY the filtered unique index (the “one true rule”)
     ...
     ;
            ...
            modelBuilder.Entity<CustomFieldDefinition>(e =>
            {
                ...
                e.Property(x => x.DataType).HasConversion<string>().HasMaxLength(40).IsRequired();

                e.Property(x => x.OptionsJson).HasColumnType("jsonb");

                // Multi-tenant uniqueness: one key per entity type per business
                e.HasIndex(x => new { x.BusinessId, x.EntityType, x.Key })
                 .IsUnique()
                 .HasDatabaseName("UX_CustomFieldDefinitions_Biz_Entity_Key");

                // common list query
                e.HasIndex(x => new { x.BusinessId, x.EntityType, x.IsActive })
            ...
            modelBuilder.Entity<CustomFieldValue>(e =>
            {
                ...
                // Uniqueness: only one value per (Business, EntityType, EntityId, FieldId)
                e.HasIndex(x => new { x.BusinessId, x.EntityType, x.EntityId, x.FieldId })
                 .IsUnique()
                 .HasDatabaseName("UX_CustomFieldValues_Biz_Entity_Record_Field");

                // fast fetch values for an entity record
                e.HasIndex(x => new { x.BusinessId, x.EntityType, x.EntityId })
            ...
            modelBuilder.Entity<Role>(entity =>
            {
                ...
                // ✅ Postgres uniqueness with NULLs needs FILTERED indexes:
                // 1) System roles: unique by Name when BusinessId IS NULL
                entity.HasIndex(r => r.Name)
                      .IsUnique()
                      .HasFilter("\"BusinessId\" IS NULL");

                // 2) Business roles: unique by (BusinessId, Name) when BusinessId IS NOT NULL
                entity.HasIndex(r => new { r.BusinessId, r.Name })
                      .IsUnique()
                      .HasFilter("\"BusinessId\" IS NOT NULL");
            });

            // ───────────────────────── Permission Overrides (Business-level) ─────────────────────────
            modelBuilder.Entity<BusinessPermissionOverride>(e =>
            {
                e.ToTable("BusinessPermissionOverrides");
                e.HasKey(x => x.Id);

                e.Property(x => x.Reason).HasMaxLength(1000);

                // 1 row per (Business, Permission)
                e.HasIndex(x => new { x.BusinessId, x.PermissionId })
                 .IsUnique()
                 .HasDatabaseName("UX_BizPermOverrides_Business_Permission");

                // Useful reads
                e.HasIndex(x => new { x.BusinessId, x.IsRevoked })
            ...
            modelBuilder.Entity<MessageLog>(e =>
            {
                ...
                e.HasIndex(x => new { x.BusinessId, x.ContactId, x.IsIncoming, x.CreatedAt });
            });
            //ContactRead (correctness: avoid duplicates)
            modelBuilder.Entity<ContactRead>(e =>
            {
                e.HasIndex(x => new { x.BusinessId, x.UserId, x.ContactId })
                 .IsUnique();
            });
            //Contact(speed: resolve contactId by phone)
            modelBuilder.Entity<Contact>(e =>
            {
                e.HasIndex(x => new { x.BusinessId, x.PhoneNumber });
                e.HasIndex(x => new { x.BusinessId, x.Id }).HasDatabaseName("ix_contacts_biz_id");
            });

            // ChatSessionState (speed: load by business+contact)

            modelBuilder.Entity<ChatSessionState>(e =>
            {
                e.HasIndex(x => new { x.BusinessId, x.ContactId });
            });
            // 📄 In AppDbContext OnModelCreating (or dedicated config file)
            modelBuilder.Entity<CampaignAudienceAttachment>(e =>
            {
                e.ToTable("CampaignAudienceAttachments");
                e.HasKey(x => x.Id);

                // ✅ only one active attachment per campaign per business
                ...
                e.HasOne(x => x.Campaign)
                 .WithMany(c => c.AudienceAttachments)
                ...
                e.HasOne(x => x.Audience)
                 .WithMany()
                 .HasForeignKey(x => x.AudienceId)
                 .OnDelete(DeleteBehavior.Restrict); // 👈 keep history safe
